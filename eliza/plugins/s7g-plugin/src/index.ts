...
const S7G_API = process.env.S7G_API_URL || "http://localhost:8080";
...
function makeAction(name: string, desc: string, task: string): Action {
  return {
    ...
    description: desc,
    validate: async () => true,
    handler: async (runtime: IAgentRuntime, message: Memory, state: State, options: any, cb?: HandlerCallback) => {
      try {
    ...
    examples: [[{ user: "user", content: { text: desc } }]],
  };
}

export const s7gPlugin: Plugin = {
  name: "s7g",
  description: "Sovereign 7G Network — 26 agents, DeFi, DePIN, cross-chain",
  actions: [
