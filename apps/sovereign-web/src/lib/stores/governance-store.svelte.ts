...
class GovernanceEngine {
    state = $state<GovernanceState>(initialState);
    ...
    private persist() {
        if (browser) saveToStorage('governance', $state.snapshot(this.state));
    }
    ...
    castVote(proposalId: string, support: 'for' | 'against' | 'abstain') {
        ...
        if (proposalIndex === -1 || this.state.userVotes[proposalId]) return;

        const proposal = this.state.proposals[proposalIndex];
        if (support === 'for') proposal.votesFor++;
        if (support === 'against') proposal.votesAgainst++;
        if (support === 'abstain') proposal.votesAbstain++;

        this.state.userVotes[proposalId] = support;
        this.state.participationScore = Math.min(100, this.state.participationScore + 1);
        ...
        this.state.activityFeed = [newActivity, ...this.state.activityFeed].slice(0, 50);
        this.persist();
    }
    ...
    createProposal(proposal: Omit<Proposal, 'id' | 'status' | 'proposer' | 'proposerName' | 'votesFor' | 'votesAgainst' | 'votesAbstain' | 'quorum' | 'startTime' | 'endTime' | 'createdAt'>) {
        ...
        this.state.proposals = [newProposal, ...this.state.proposals];
        this.state.participationScore = Math.min(100, this.state.participationScore + 5);
        this.state.activityFeed = [newActivity, ...this.state.activityFeed].slice(0, 50);
        this.state.governanceStats.totalProposals++;
    ...
    delegateVotes(delegateAddress: string) {
        ...
        this.state.userDelegates = delegateAddress;
        this.state.delegationHistory.push({
            to: delegateAddress,
            timestamp: new Date(),
            amount: this.state.votingPower
        });
    ...
    getUserVotingPower(resonance: number, holdings: number): number {
...
export const governanceStore = new GovernanceEngine();
