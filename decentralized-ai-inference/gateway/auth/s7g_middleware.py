...
async def custom_auth_fn(*args, **kwargs) -> dict:
    """LiteLLM custom auth hook.
    
    Submits a consensus proposal to the S7G committee node before executing inference.
    """
    # 1. Determine if called by LiteLLM (FastAPI request) or unit tests
    is_fastapi_request = False
    if args and hasattr(args[0], "url"):
        is_fastapi_request = True
        request = args[0]
        path = request.url.path
    else:
        path = kwargs.get("method") or (args[2] if len(args) > 2 else "/chat/completions")
        request_body = kwargs.get("request_body") or (args[1] if len(args) > 1 else {})

    # 2. Only intercept chat completions requests
    if "/chat/completions" not in path:
        return {"decision": True}

    # 3. For FastAPI requests, read the JSON body asynchronously
    if is_fastapi_request:
        try:
...
import litellm
litellm.callbacks.append(S7GLoggingHandler())
